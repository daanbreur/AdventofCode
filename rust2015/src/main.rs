...
use rust2015::{run_day, set_verbosity, test_day, Part, Verbosity};
...
fn main() {
    let matches = App::new("Advent Of Code 2015")
        ...
        .about("My Advent Of Code 2015 solutions.")
...
fn download_input(day: u8, session: &String) -> Result<String, reqwest::Error> {
    ...
    let response = client
        .get(format!("https://adventofcode.com/2015/day/{}/input", day))
        .header(USER_AGENT, "https://github.com/daanbreur/AdventofCode by daan@daanbreur.systems")
        .send()?;
...
fn get_auto_input(day: u8, session: &String, cache: bool) -> String {
    let cache_str = &format!("./.aoc15_cache/input{:02}.txt", day);
    ...
    match cache {
        true => match fs::read_to_string(cache_path) {
            ...
            Err(_) => match download_input(day, session) {
                Ok(input) => {
                    let _ = fs::create_dir(Path::new("./.aoc15_cache"));
